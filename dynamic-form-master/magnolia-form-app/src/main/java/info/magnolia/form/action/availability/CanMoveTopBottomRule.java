 ...
 * @param <T> Type.
 */
public class CanMoveTopBottomRule<T> extends AbstractAvailabilityRule<T, CanMoveTopBottomRuleDefinition> {

    private final DbItemResolver itemResolver;
    private final SubAppContext subAppContext;

    @Inject
    public CanMoveTopBottomRule(AvailabilityDefinition availabilityDefinition, CanMoveTopBottomRuleDefinition ruleDefinition, DbItemResolver itemResolver, SubAppContext subAppContext) {
        ...
        return Optional.of(subAppContext)
                ...
                .map(path -> {
                    ...
                    if (optionalItemToMove.isPresent()) {
                        ...
                        if ((itemToMove instanceof Form)) {
                            return false;
                        }
                        if ((itemToMove instanceof Section) && !(target instanceof Form)) {
                            return false;
                        }
                        if ((itemToMove instanceof Question) && !((target instanceof Section) || (target instanceof Form))) {
                            return false;
                        }
                        if ((itemToMove instanceof AnswerOption) && !(target instanceof Question)) {
                            return false;
                        }
                        return true;
                    }
